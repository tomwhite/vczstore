...
def test_copy_then_append_azure_icechunk(tmp_path, azurite_env):
    vcz1 = _convert_vcf_to_vcz_zarr3("sample-part1.vcf.gz", tmp_path)
    vcz2 = _convert_vcf_to_vcz_zarr3("sample-part2.vcf.gz", tmp_path)
    expected = _convert_vcf_to_vcz_zarr3("sample.vcf.gz", tmp_path)

    azure_url = f"az://{azurite_env['account']}/{azurite_env['container']}/store.vcz"

    copy_store_to_icechunk(vcz1, azure_url)
    with icechunk_transaction(azure_url, "main", message="append") as store:
        append(store, vcz2)

    repo = Repository.open(make_icechunk_storage(azure_url))
    session = repo.readonly_session("main")
    actual_root = zarr.open_group(store=session.store, mode="r")
    expected_root = zarr.open_group(store=expected, mode="r")
